 ...
 */
 ...
 function SSE(req, res, next) {
  ...
  res.sse = function (eventObject, options) {
    console.log('[SSE] EVENT '.cyan, req.query.pollKey, Object.keys (eventObject));
    var options = options || {};
    ...
    if(!res.headersSent){
      ...
      res.write("retry: " + ( options.retry || 1000 )+ "\n");
    }
    ...
    if (_.isString(eventObject)) {
     ...
     if(options.send){
      console.log('[SSE]', 'not just flushing im sending it to' );
      res.send();
    }
  }
};
